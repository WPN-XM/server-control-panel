...
PluginListDelegate::PluginListDelegate(QListWidget *parent) : QStyledItemDelegate(parent), m_rowHeight(0), m_padding(0)
{
}
...
void PluginListDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    // Draw version
    const int leftPosForVersion = titleMetrics.boundingRect(name).width() + m_padding;
    const QString version       = index.data(Qt::UserRole).toString();
    QRect versionRect(nameRect.x() + leftPosForVersion, nameRect.y(), rightTitleEdge - leftPosForVersion,
                      titleMetrics.height());
    ...
    const QString description =
        opt.fontMetrics.elidedText(index.data(Qt::UserRole + 2).toString(), Qt::ElideRight, descriptionRect.width());
    painter->setFont(opt.font);
    style->drawItemText(painter, descriptionRect, Qt::TextSingleLine | Qt::AlignLeft, textPalette, true, description,
                        colorRole);
}

...
namespace PluginsNS
{
    ...
    PluginListDelegate::PluginListDelegate(QListWidget *parent)
        : QStyledItemDelegate(parent), m_rowHeight(0), m_padding(0)
    {
    }
                                   ...
                                   const QModelIndex &index) const
    {
        ...
        QRect nameRect(leftTitleEdge, opt.rect.top() + m_padding, rightTitleEdge - leftTitleEdge,
                       titleMetrics.height());
    ...
    QSize PluginListDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
    {
        ...
        if (!m_rowHeight) {
            ...
            m_rowHeight =
                2 * m_padding + 2 * opt.fontMetrics.leading() + opt.fontMetrics.height() + titleMetrics.height();
        }
